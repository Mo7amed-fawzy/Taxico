...
class PriceDetailModel {
  ...
  static Future<List> getList() async {
    ...
    if (db != null) {
      List<Map> list = await db.rawQuery(
          'SELECT * FROM `${AppTexts.tbPriceDetail}` WHERE `${AppTexts.status}` = 1',
          []);
      return list;
    } else {
      return [];
  ...
  static Future<List> getSelectZoneGetServiceAndPriceList(String zoneId) async {
    ...
    if (db != null) {
      List<Map> list = await db.rawQuery(
          'SELECT `sd`.`service_id`, `pd`.`price_id`, `pd`.`base_charge`, `pd`.`per_km_charge`, `pd`.`per_min_charge`, `pd`.`booking_charge`, `pd`.`mini_fair`, `pd`.`mini_km`, `sd`.`service_name`, `sd`.`color`, `sd`.`icon` FROM `${AppTexts.tbServiceDetail}` AS `sd` INNER JOIN `${AppTexts.tbPriceDetail}` AS `pd` ON `pd`.`${AppTexts.serviceId}` = `sd`.`${AppTexts.serviceId}` AND `sd`.`${AppTexts.status}` = 1 AND `pd`.`${AppTexts.status}` = 1 AND ( `sd`.`${AppTexts.gender}` LIKE ? ) WHERE `pd`.`${AppTexts.zoneId}` = ?',
