...
import 'package:taxi_grad/core/widgets/location_select_button.dart';
import 'package:taxi_grad/core/widgets/round_button.dart';
import 'package:taxi_grad/features/menu/menu_view.dart';
import 'package:taxi_grad/features/user/car_service_select_view.dart';
...
class _UserHomeViewState extends State<UserHomeView> {
  ...
  void getSelectLocation(bool isPickup) async {
    ...
    printHere("lat: ${centerMap.latitude}, long:${centerMap.longitude}");
    ...
    if (addressArr.isNotEmpty) {
      if (isPickup) {
        ...
        printHere(pickupAddressObj.toString());
        ...
        printHere(dropAddressObj.toString());
    ...
    if (isPickup) {
      ...
      for (var zmObj in zoneListArr) {
        if (PolygonUtil.containsLocation(
          ...
          selectZone = zmObj;
          printHere("Found Inside Zone -------");

          printHere(zmObj.toMap().toString());
        }
      }

      if (selectZone == null) {
        printHere("Not Found Inside Zone -------");
      }
    }
  ...
  void apiHome() {
    ...
    ServiceCall.post({}, SVKey.svHome, isTokenApi: true,
      ...
      Globs.hideHUD();
      mdShowAlert(AppTexts.appName, error.toString(), () {});
    });
  }
}
