...
import 'package:taxi_grad/core/widgets/support_user_row.dart';
...
class _SupportListViewState extends State<SupportListView> {
  ...
  void initState() {
    ...
    SocketManager.shared.socket?.on("support_message", (data) {
      printHere("support_message socket get :${data.toString()} ");
  ...
  void getList() {
    ...
    ServiceCall.post({
      ...
      Globs.hideHUD();
      mdShowAlert(AppTexts.appName, error as String? ?? MSG.fail, () {});
    });
  }
}
